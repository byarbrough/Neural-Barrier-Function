...
def attack(model_ori, x, data_lb, data_ub, vnnlib, verified_status, verified_success, crown_filtered_constraints=None,
    ...
    if 'auto_attack' not in arguments.Config["attack"]["attack_mode"]:
        ...
        if crown_filtered_constraints is not None:
            ...
            for i in range(len(list_target_label_arrays[0])):
                if crown_filtered_constraints[i] == True: continue
        ...
        attack_ret, attack_images, attack_margins, all_adv_candidates = attack_with_general_specs(model_ori, x,
                                                                                                  data_min_repeat[:,
                                                                                                  :len(
                                                                                                      list_target_label_arrays[
                                                                                                          0]), ...],
    ...
    if attack_ret:
        ...
        if arguments.Config["general"]["save_adv_example"]:
            save_cex(attack_images, attack_output, x, vnnlib, arguments.Config["attack"]["cex_path"], data_max_repeat,
                     data_min_repeat)
...
def build_loss(origin_out, output, C_mat, rhs_mat, cond_mat, same_number_const, gama_lambda=0, threshold=-1e-5,
               mode='hinge'):
    '''
...
def pgd_attack_with_general_specs(model, X, data_min, data_max, C_mat, rhs_mat, cond_mat, same_number_const, alpha,
                                  use_adam=True, normalize=lambda x: x,
                                  initialization='uniform', GAMA_loss=False, set_num_restarts=None, set_pgd_steps=None,
    ...
    '''
    device = X.device
    attack_iters = arguments.Config["attack"]["pgd_steps"] if set_pgd_steps is None else set_pgd_steps
    num_restarts = arguments.Config["attack"]["pgd_restarts"] if set_num_restarts is None else set_num_restarts
    ...
    if initialization == 'boundary':
        boundary_adv_examples = boundary_attack(model, X[:, 0, ...].view(-1, *input_shape[1:]),
                                                data_min.view(*input_shape), data_max.view(*input_shape))
    ...
    if initialization == 'osi' or initialization == 'boundary':
        ...
        delta = (torch.empty_like(X).uniform_() * (
                    delta_upper_limit - delta_lower_limit) + delta_lower_limit).requires_grad_()
    ...
    for _ in range(attack_iters):
        ...
        loss, loss_gama = build_loss(origin_out, output, C_mat, rhs_mat, cond_mat, same_number_const,
                                     gama_lambda if GAMA_loss else 0.0,
                                     mode=arguments.Config['attack']['pgd_loss_mode'])
        ...
        with torch.no_grad():
            ...
            delta_targeted = delta.view(delta.size(0), -1, *input_shape[1:]).gather(dim=1, index=indices.view(-1, 1,
                                                                                                              *(1,) * (
                                                                                                                          len(input_shape) - 1)).expand(
                -1, -1, *input_shape[1:])).squeeze(1)
...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, num_restarts,
    ...
    if initialization == 'osi':
        ...
        X_init = OSI_init(model, X, y, epsilon, alpha, num_classes, iter_steps=attack_iters, extra_dim=extra_dim,
        ...
        for _ in range(attack_iters):
            ...
            with torch.no_grad():
                ...
                if not multi_targeted:
                    ...
                    if nn4sys:
                        all_loss, indices = loss.max(1)
                        delta_best = delta.gather(dim=1,
                                                  index=indices.view(-1, 1, *[1] * len(input_shape[1:])).expand(-1, -1,
                                                                                                                *input_shape[
                                                                                                                 1:])).squeeze(
                            1)
                        ...
                        delta_best = delta.gather(dim=1, index=indices.view(-1, 1, 1, 1, 1).expand(-1, -1, *input_shape[
                                                                                                            1:])).squeeze(
                            1)
                        best_delta[all_loss >= best_loss] = delta_best[all_loss >= best_loss]
                        best_loss = torch.max(best_loss, all_loss)
                    ...
                    # delta has shape (batch, restarts, num_class-1, c, h, w). For each batch element, we want to select from the best over (restarts, num_classes-1) dimension.
                    # delta_targeted has shape (batch, c, h, w).
                    delta_targeted = delta.view(delta.size(0), -1, *input_shape[1:]).gather(dim=1,
...
class AdamClipping(Optimizer):
    ...
    @torch.no_grad()
    @torch.jit.script
    def _clip_update(exp_avg: torch.Tensor, denom: torch.Tensor, step_size: float, clipping_step_eps: float,
                     lower_limit: torch.Tensor, upper_limit: torch.Tensor, p: torch.Tensor):
...
def attack_with_general_specs(model, x, data_min, data_max, list_target_label_arrays, initialization="uniform",
                              GAMA_loss=False):
    ...
    print(
        f'Attack parameters: initialization={initialization}, steps={arguments.Config["attack"]["pgd_steps"]}, restarts={arguments.Config["attack"]["pgd_restarts"]}, alpha={alpha}, initialization={initialization}, GAMA={GAMA_loss}')
    ...
    # if test_conditions(x, output, C_mat, rhs_mat, cond_mat, same_number_const, data_max.unsqueeze(1),
    #                    data_min.unsqueeze(1)).all():
    #     print("Clean prediction incorrect, attack skipped.")
    #     # Obtain attack margin.
    #     attack_image, _, attack_margin = gen_adv_example(model, x, torch.zeros_like(x), data_max, data_min, C_mat,
    #                                                      rhs_mat, cond_mat)
    ...
    best_deltas, last_deltas = pgd_attack_with_general_specs(model, x, data_min, data_max, C_mat, rhs_mat, cond_mat,
                                                             same_number_const, alpha,
                                                             initialization=initialization, GAMA_loss=GAMA_loss)

    attack_image, attack_output, attack_margin = gen_adv_example(model, x, best_deltas, data_max, data_min, C_mat,
                                                                 rhs_mat, cond_mat)
    ...
    if test_conditions(attack_image.unsqueeze(1), attack_output.unsqueeze(1), C_mat, rhs_mat, cond_mat,
...
def pgd_attack(dataset, model, x, max_eps, data_min, data_max, vnnlib=None, y=None,
    ...
    if dataset in ["MNIST", "CIFAR",
                   "UNKNOWN"]:  # FIXME (01/11/2022): Make the attack function generic, not for the two datasets only!
        ...
        for i, spec in enumerate(specs):
            ...
            print('##### PGD attack: Batch {}, Threshold: {} ######'.format(i, y_upper[i][:10].squeeze() * y_sign[i][
                                                                                                           :10].squeeze()),
                  y_sign[i][:10].squeeze())
            ...
            best_deltas, last_deltas = attack_pgd(model, X=x, y=y, target=target, epsilon=float("inf"), alpha=alpha,
                                                  num_classes=arguments.Config["data"]["num_outputs"],
                                                  attack_iters=arguments.Config["attack"]["pgd_steps"],
                                                  num_restarts=arguments.Config["attack"]["pgd_restarts"],
                                                  upper_limit=data_max, lower_limit=data_min,
                                                  ...
                                                  initialization=initialization,
                                                  early_stop=arguments.Config["attack"]["pgd_early_stop"],
                                                  GAMA_loss=GAMA_loss, nn4sys=True)
