...
def batch_verification(d, net, batch, pre_relu_indices, growth_rate, fix_intermediate_layer_bounds=True,
    ...
    if mask is not None:
        ...
        if len(branching_decision) < len(mask[0]):
            ...
            if not arguments.Config["solver"]["beta-crown"]["all_node_split_LP"]:
                ...
                if len(d) > 0:
                    ...
                    global_lb_2 = worst_domain[-1].lower_bound - worst_domain[-1].threshold
                    if max(global_lb_2) <= max(global_lb):
                        global_lb = global_lb_2
                # Fixme: changes made here
                return global_lb, np.inf, None
        ...
        # TODO: figure out why len(dom_lb) = 128 while the num_copy = 64?
    ...
    if len(d) > 0:
        if arguments.Config["bab"]["cut"]["enabled"] and arguments.Config["bab"]["cut"]["cplex_cuts"] and arguments.Config["bab"]["cut"]["cplex_cuts_revpickup"]:
            worst_domain = d.get_min_domain(1 ,rev_order=True)
            # FIXME: the global lower bounds are offset by the thresholds
            global_lb = worst_domain[-1].lower_bound

            # global_lb = worst_domain[-1].lower_bound - worst_domain[-1].threshold
        else:
            worst_domain = d.get_min_domain(1 ,rev_order=False)
            global_lb = worst_domain[0].lower_bound

            # global_lb = worst_domain[0].lower_bound - worst_domain[0].threshold
    else:
        print("No domains left, verification finished!")
        print('{} domains visited'.format(Visited))
        return torch.tensor(arguments.Config["bab"]["decision_thresh"] + 1e-7), np.inf, (dom_ub, primals)

    batch_ub = np.inf
    # Fixme: changes made in this step
    dom_ub_batch = dom_ub
    if get_upper_bound:
        batch_ub = min(dom_ub)
    ...
    print('{} domains visited'.format(Visited))
    return global_lb, batch_ub, (dom_ub_batch, primals)
...
def relu_bab_parallel(net, domain, x, use_neuron_set_strategy=False, refined_lower_bounds=None,
    ...
    # FIXME: add pool size
    adv_samples_pool_size = arguments.Config["alg_options"]["bab"]["adv_samples_pool_size"]
    ...
    # TODO: why do we extract the decision threshold from the lower bounds? To compare with zero threshold later on
    ...
    # FIXME: initialize the sample pool (upper bounds, primals)
    adv_samples_pool = None
    ...
    while run_condition:
        ...
        if global_lb is None:
            ...
            if len(domains) > 80000 and len(domains) % 10000 < batch * 2 and use_neuron_set_strategy:
                ...
                global_lb, batch_ub, ub_result = batch_verification(domains, net, int(batch/2), pre_relu_indices, 0,
                                        fix_intermediate_layer_bounds=False, stop_func=stop_criterion,
                                        multi_spec_keep_func=multi_spec_keep_func)
            else:
                # TODO: check if we should use stop_criterion(decision_thresh)
                global_lb, batch_ub, ub_result = batch_verification(domains, net, batch, pre_relu_indices, 0,
        ...
        # FIXME: update the adversarial sample pool
        if adv_samples_pool is None:
            if ub_result is not None and ub_result[1] is not None:
                adv_samples_pool = ub_result
                ind = torch.argsort(adv_samples_pool[0].view(-1))[:adv_samples_pool_size]
                adv_samples_pool = (adv_samples_pool[0][ind], adv_samples_pool[1][ind])
        else:
            if ub_result is not None and ub_result[1] is not None:
                adv_samples_pool = (torch.cat((adv_samples_pool[0], ub_result[0]), dim=0),
                                    torch.cat((adv_samples_pool[1], ub_result[1]), dim=0))
                ind = torch.argsort(adv_samples_pool[0].view(-1))[:adv_samples_pool_size]
                adv_samples_pool = (adv_samples_pool[0][ind], adv_samples_pool[1][ind])
        ...
        if all_node_split:
            ...
            return global_lb, global_ub, glb_record, Visited, adv_samples_pool,'unknown'
        ...
        if get_upper_bound or arguments.Config["bab"]["attack"]["enabled"]:
            if global_ub < decision_thresh:
                ...
                glb_record.append([time.time() - start, global_lb])
                return global_lb, global_ub, glb_record, Visited, adv_samples_pool, 'unsafe'
    ...
    if arguments.Config["bab"]["attack"]["enabled"]:
        ...
        return global_lb, global_ub, glb_record, Visited, adv_samples_pool, 'safe'
