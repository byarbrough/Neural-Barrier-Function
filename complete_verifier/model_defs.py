...
class Step_carvana(nn.Module):
    def __init__(self, ori_carvana, gt):
        ...
        gt = torch.tensor(gt, dtype=torch.get_default_dtype()).reshape(1, 31,
                                                                       47)  # 0 means 0-dim > 1-dim, 1 means 0-dim < 1-dim
        ...
        self.step_value_zero = torch.nn.Parameter(torch.tensor(0., dtype=torch.get_default_dtype()),
                                                  requires_grad=False)
