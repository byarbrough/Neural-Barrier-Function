...
class ConfigHandler:
    ...
    def add_common_options(self):
        ...
        h = ["alg_options"]
        self.add_argument("--train_method", type=str, default="verification-only", choices=["verification-only", "fine-tuning"],
                          help='Select the training framework for the barrier function.', hierarchy=h+["train_method"])
        self.add_argument("--verification_method", type=str, default="bab", choices=["bab", "mip"],
                          help="Choose the verification method in the training loop.", hierarchy=h+["verification_method"])
        ...
        self.add_argument("--early_stopping_tol", type=float, default=0.000000001, hierarchy=h+["early_stopping_tol"])
        self.add_argument("--update_A_freq", type=int, default=1, hierarchy=h+["update_A_freq"])
        self.add_argument("--samples_pool_size", type=int, default=100000, hierarchy=h+["samples_pool_size"])
        self.add_argument("--B_batch_size", type=int, default=50, hierarchy=h+["B_batch_size"])
        self.add_argument("--num_iter", type=int, default=20, hierarchy=h+["num_iter"])
        self.add_argument("--scaling_factor", type=float, default=1.0, hierarchy=h+["scaling_factor"])
        self.add_argument("--noise_weight", type=float, default=0.0, hierarchy=h+["noise_weight"])
        self.add_argument("--train_timeout", type=float, default=3600, hierarchy=h+["train_timeout"])
        self.add_argument("--condition_tol", type=float, default=0.00001, hierarchy=h+["condition_tol"])

        h = ["alg_options", "ce_sampling"]
        self.add_argument("--num_ce_samples", type=int, default=20, hierarchy=h+["num_ce_samples"])
        self.add_argument("--num_ce_samples_accpm", type=int, default=5, hierarchy=h+["num_ce_samples_accpm"])
        ...
        self.add_argument("--cvxpy_solver", type=str, default='ECOS', hierarchy=h+["cvxpy_solver"])
        self.add_argument("--num_ce_thresh", type=int, default=5, hierarchy=h+["num_ce_thresh"])

        h = ["alg_options", "bab"]
        self.add_argument("--bab_yaml_path", type=str, default='double_integrator.yaml', hierarchy=h+["bab_yaml_path"])
        self.add_argument("--adv_sample_filter_radius_x0", type=float, default=0.05, hierarchy=h+["adv_sample_filter_radius_x0"])
        self.add_argument("--adv_sample_filter_radius_xu", type=float, default=0.05, hierarchy=h+["adv_sample_filter_radius_xu"])
        self.add_argument("--adv_sample_filter_radius_x", type=float, default=0.05, hierarchy=h+["adv_sample_filter_radius_x"])
        self.add_argument('--adv_samples_pool_size', type=int, default=50,
                          help="Number of attack samples stored.", hierarchy=h+["adv_samples_pool_size"])
        self.add_argument("--get_upper_bound_samples", type=int, default=2048, hierarchy=h+["get_upper_bound_samples"])

        h = ["alg_options", "mip"]
        self.add_argument("--time_limit", type=float, default=0.0, hierarchy=h+["time_limit"])
        self.add_argument("--MIPFocus", type=int, default=0, hierarchy=h+["MIPFocus"])
        ...
        self.add_argument("--model", type=str, default=None,
                          help='Model name. Will be evaluated as a python statement.',
                          hierarchy=h + ["name"])
        ...
        self.add_argument('--mip_solver', type=str, default='gurobi',
                          help="MLP/LP solver package (SCIP support is experimental).",
        ...
        self.add_argument('--cut_method', help='Cutting plane generation method (unused, for future extensions).',
                          hierarchy=h + ["method"])
        ...
        self.add_argument('--pgd_order', choices=["before", "after", "middle", "skip"], default="before",
                          help='Run PGD attack before/after incomplete verification, or skip it.',
                          hierarchy=h + ["pgd_order"])
        ...
        self.add_argument('--input_split_pgd_restarts', type=int, default=30,
                          help="Number of random PGD restarts in input split before branching starts.",
                          hierarchy=h + ["pgd_restarts"])
