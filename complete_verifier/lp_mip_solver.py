...
import complete_verifier.beta_CROWN_solver as beta_CROWN_solver
...
def mip_solver(candidate, init=None):
    ...
    print(
        "Solving MIP for {}, [{},{}]=>[{},{}] ({},{}; {},{}), time: {:.4f}s, #vars: {}, #constrs: {}, improved: {}".format(
            v.VarName, out_lb, out_ub, vlb, vub,
            status_lb, status_lb_r, status_ub, status_ub_r, time.time() - refine_time, model.NumVars, model.NumConstrs,
            neuron_refined))
    sys.stdout.flush()
...
def mip_solver_attack(new_splits):
    ...
    if termination_flag.value == 1:
        return float("inf"), float("inf"), -1, torch.zeros([0, ] + input_shape[1:],
                                                           requires_grad=False)  # Solver skipped.
    ...
    print("Solved MIP for {}, {} neurons fixed, status:{} ({} sols), lower={:.6f}, upper={:.6f}, time: {:.6f}s".format(
        v.VarName, len(indices), status,
...
def mip_solver_lb_ub(candidate, init=None, save_adv=None):
    ...
    if arguments.Config["solver"]["mip"]["early_stop"]:
        if arguments.Config["solver"]["mip"]["mip_solver"] == 'gurobi':
            ...
            model.includeEventhdlr(EarlyStopEvent(), "EarlyStopEvent",
                                   "python event handler to early stop.")  # FIXME (01/13/22): threshold should be changable, based on specification.
        else:
...
def lp_solver(candidate):
    ...
    print_str = "Linear {}: old_lb={:.7g}, new_lb={:.7g}, old_ub={:.7g}, new_ub={:.7g}, lb_diff={:.7g}, ub_diff={:.7g}, time={:3g}".format(
        v.VarName, out_lb, vlb, out_ub, vub, vlb - out_lb, out_ub - vub, time.time() - refine_time)
    print(print_str)
    sys.stdout.flush()
...
def build_solver_model(m, timeout, mip_multi_proc=None,
    ...
    print(
        f"mip_multi_proc: {mip_multi_proc}, mip_threads: {mip_threads}, total threads used: {mip_multi_proc * mip_threads}")
    build_mip_time = time.time()

    # build model in auto_LiRPA
    out_vars = m.net.build_solver_module(
        x=x, C=m.c, intermediate_layer_bounds=intermediate_bounds, final_node_name=m.net.final_name,
        model_type=model_type, solver_pkg=m.net.solver_pkg)
    m.net.model.update()
...
def all_node_split_LP(m, lower_bounds, upper_bounds, rhs):
    ...
    for out_idx in range(len(orig_out_vars)):
        ...
        if m.all_node_model.status == 2:
            ...
            glb = float('inf')
        print(
            f"# all node split glb [{objVar.VarName}], status {m.all_node_model.status}: {glbs[0]} -> {glb} vs rhs {decision_threshold}")
...
def construct_mip_with_model(unwrapped_model, x, input_shape, c, intermediate_bounds,
    ...
    build_the_model_mip(model, labels_to_verify=None, save_mps=save_mps, process_dict=process_dict, x=x,
                        intermediate_bounds=intermediate_bounds)


# updated function using general computation graph to build lp model
# @torch.no_grad()
def build_the_model_mip(m, labels_to_verify=None, save_mps=False, process_dict=None, save_adv=False, x=None,
    ...
    if save_mps:
        ...
        for cidx, candidate in enumerate(candidates):
            ...
            model_filename = os.path.join(CPLEX_FOLDER,
        ...
        for pidx in model_filename_stamped_dict:
            ...
            try:
                ...
                processes[pidx] = {'pid': proc.pid, '_logfile': logfile, '_fname_stamped': model_filename_stamped,
                                   'c': model_c_row}
...
def run_get_cuts_subprocess(model_filename):
    ...
    return subprocess.Popen([f"{CPLEX_FOLDER}/get_cuts", f"{model_filename}.mps", f"{model_filename}"],
                            ...
                            stdout=logfile), logfile.fileno()  # Also returns the logfile handle so it can be closed later.
        ...
        if type(layer) is nn.Linear:
            ...
            for neuron_idx in range(len(xs)):
                ...
                if lb < 0 and ub > 0:
                    ...
                    pi = orig_model.getConstrByName(
                        f'ReLU{relu_idx}_{neuron_idx}_a_0').pi  # dual variable for upper bound \hat{x} <= x - l + l * z
                    gamma = orig_model.getConstrByName(
                        f'ReLU{relu_idx}_{neuron_idx}_a_1').pi  # dual variable for lower bound \hat{x} >= x.
                    tau = orig_model.getConstrByName(
                        f'ReLU{relu_idx}_{neuron_idx}_a_2').pi  # dual variable for another upper bound \hat{x} <= u * z
                    mu = orig_model.getConstrByName(
                        f'ReLU{relu_idx}_{neuron_idx}_a_3').pi  # dual variable for lower bound \hat{x} >= 0.
                    ...
                    print(
                        f'layer {i:2d} neuron {neuron_idx:3d} l={lb:8.5f} ub={ub:8.5f} x={x:8.5f} hat_x={hat_x:8.5f} pi={pi:8.5f} ({new_pi:8.5f}), tau={tau:8.5f} ({new_tau:8.5f}), mu={mu:8.5f}, gamma={gamma:8.5f}, nu_hat={nu_hat:8.5f}, alpha={alpha:8.5f} z={z:8.5f} upper_z={upper_z:8.5f} lower_z={lower_z:8.5f} {neuron_set_name}')
                    unstable_idx += 1
            relu_idx += 1
...
def update_mip_model_fix_relu(m, relu_idx, status, target=None, async_mip=False, best_adv=None,
...
def build_the_model_mip_refine(m, lower_bounds, upper_bounds,
    ...
    mip_refine_timeout = arguments.Config["solver"]["mip"]["refine_neuron_time_percentage"] * arguments.Config["bab"][
        "timeout"]
    ...
    m.net.set_bound_opts(
        {'optimize_bound_args': {'iteration': 100, 'enable_beta_crown': False, 'enable_alpha_crown': True,
    ...
    for layer in m.layers:
        ...
        if type(layer) is nn.Linear:
            ...
            for neuron_idx in range(weight.size(0)):
                ...
                if (relu_idx >= 1 and relu_idx < len(m.net.relus)) and (out_lb * out_ub < 0) and (
                        time.time() - mip_refine_time_start < mip_refine_timeout):
            ...
            refine_time = time.time()
            if need_refine and (relu_idx >= 1 and relu_idx < len(m.net.relus)) and (
                    time.time() - mip_refine_time_start < mip_refine_timeout):
                ...
                if relu_idx == 1:
                    if adv_warmup:
                        ...
                        adv, max_values, min_values = _intermediate_PGD_attack(m, relu_idx, restarts=3, attack_iters=50,
                                                                               alpha=None)
                        adv_list = []
                        layer_size = weight.size(0)
                        for neuron_idx in candidate_neuron_ids:
                            adv_list.append((adv[neuron_idx + layer_size].cpu().tolist(),
                                             adv[neuron_idx].cpu().tolist()))  # (low adv, max adv)
                        ...
                        with multiprocessing.Pool(mip_multi_proc) as pool:
                            ...
                            if relu_idx + 1 < len(m.net.relus):
                                ...
                                adv, max_values, min_values = _intermediate_PGD_attack(m, relu_idx + 1, restarts=3,
                                                                                       attack_iters=50, alpha=None)
                            solver_result = solver_result.get()
                    else:
                        # the second relu layer where mip refine starts
                        with multiprocessing.Pool(mip_multi_proc) as pool:
                    ...
                    print(
                        f"MIP improved {refined_num} nodes out of {len(candidates)} unstable nodes, lb improved {lb_refined_sum}, ub improved {ub_refined_sum}, time {refine_time:.4f}")
                    if refined_num > 0:
                    ...
                    print("maximum relu layer improved by MIP so far", maximum_refined_relu_layers,
                          "last_relu_layer_refined:", last_relu_layer_refined)
                    m.model.update()
                    ...
                    if not arguments.Config['solver']['beta-crown']['enable_opt_interm_bounds']:
                        for relu_layer in m.net.relus:
                            relu_layer.sparse_beta = torch.zeros(size=(1, 0), dtype=torch.get_default_dtype(),
                                                                 device=device, requires_grad=True)
                            relu_layer.sparse_beta_loc = torch.zeros(size=(1, 0), dtype=torch.int64, device=device,
                                                                     requires_grad=False)
                            relu_layer.sparse_beta_sign = torch.zeros(size=(1, 0), dtype=torch.get_default_dtype(),
                        ...
                        for ridx, relu_layer in enumerate(m.net.relus):
                            ...
                            for key in relu_layer.alpha.keys():
                                relu_layer.sparse_beta[key] = torch.zeros(size=(1, 0), dtype=torch.get_default_dtype(),
                                                                          device=device, requires_grad=True)
                                relu_layer.sparse_beta_loc[key] = torch.zeros(size=(1, 0), dtype=torch.int64,
                                                                              device=device, requires_grad=False)
                                relu_layer.sparse_beta_sign[key] = torch.zeros(size=(1, 0),
                    ...
                    with multiprocessing.Pool(mip_multi_proc) as pool:
                        ...
                        if last_relu_layer_refined and (time.time() - mip_refine_time_start < mip_refine_timeout):
                            ...
                            if not arguments.Config['solver']['beta-crown']['enable_opt_interm_bounds']:
                                ...
                                refined_relu_layer.sparse_beta = torch.zeros(size=(1, max_splits_per_layer),
                                                                             dtype=torch.get_default_dtype(),
                                                                             device=device, requires_grad=True)
                                refined_relu_layer.sparse_beta_loc = torch.zeros(size=(1, max_splits_per_layer),
                                                                                 dtype=torch.int64, device=device,
                                                                                 requires_grad=False)
                                refined_relu_layer.sparse_beta_sign = torch.zeros(size=(1, max_splits_per_layer),
                                                                                  dtype=torch.get_default_dtype(),
                                                                                  device=device, requires_grad=False)
                                # assign split constraint into regular betas
                                for neuron_idx, (refined_neuron, sign) in enumerate(
                                        unstable_to_stable[maximum_refined_relu_layers]):
                                    refined_relu_layer.sparse_beta_loc[0, neuron_idx] = refined_neuron
                                    refined_relu_layer.sparse_beta_sign[0, neuron_idx] = sign
                            else:
                                for key in refined_relu_layer.sparse_beta.keys():
                                    # init all intermediate betas
                                    refined_relu_layer.sparse_beta[key] = torch.zeros(size=(1, max_splits_per_layer),
                                                                                      dtype=torch.get_default_dtype(),
                                                                                      device=device, requires_grad=True)
                                    refined_relu_layer.sparse_beta_loc[key] = torch.zeros(
                                        size=(1, max_splits_per_layer), dtype=torch.int64, device=device,
                                        requires_grad=False)
                                    refined_relu_layer.sparse_beta_sign[key] = torch.zeros(
                                        size=(1, max_splits_per_layer), dtype=torch.get_default_dtype(), device=device,
                                        requires_grad=False)
                                for neuron_idx, (stable_neuron_idx, sign) in enumerate(
                                        unstable_to_stable[maximum_refined_relu_layers]):
                                    for key in refined_relu_layer.sparse_beta.keys():
                                        # assign split constraint into all intermdiate betas
                                        refined_relu_layer.sparse_beta_loc[key][0, neuron_idx] = stable_neuron_idx
                                        refined_relu_layer.sparse_beta_sign[key][0, neuron_idx] = sign
                            print("relu layer:", maximum_refined_relu_layers, "has unstable to stable neurons:",
                                  unstable_to_stable[maximum_refined_relu_layers])

                            m.net.set_bound_opts({'optimize_bound_args': {'enable_beta_crown': True, "verbose": True},
                                                  'enable_opt_interm_bounds': arguments.Config['solver']['beta-crown'][
                                                      'enable_opt_interm_bounds']})

                            lb_refined, ub_refined = m.net.compute_bounds(x=(x,),
                                                                          C=m.c, method='CROWN-optimized',
                                                                          return_A=False,
                ...
                for out_row_idx in range(out_lbs.size(2)):
                    ...
                    for out_col_idx in range(out_lbs.size(3)):
                        ...
                        for in_chan_idx in range(layer.weight.shape[1]):
                            ...
                            in_row_idx_max = -layer.padding[0] + layer.stride[0] * out_row_idx + layer.weight.shape[
                                2] - 1
                            if in_row_idx_min < 0: ker_row_min = 0 - in_row_idx_min
                            if in_row_idx_max >= pre_lb_size[2]: ker_row_max = ker_row_max - in_row_idx_max + \
                                                                               pre_lb_size[2] - 1
                            ...
                            in_col_idx_max = -layer.padding[1] + layer.stride[1] * out_col_idx + layer.weight.shape[
                                3] - 1
                            if in_col_idx_min < 0: ker_col_min = 0 - in_col_idx_min
                            if in_col_idx_max >= pre_lb_size[3]: ker_col_max = ker_col_max - in_col_idx_max + \
                                                                               pre_lb_size[3] - 1
                            in_col_idx_min, in_col_idx_max = max(in_col_idx_min, 0), min(in_col_idx_max,
                                                                                         pre_lb_size[3] - 1)

                            coeffs = layer.weight[out_chan_idx, in_chan_idx, ker_row_min:ker_row_max,
                                     ker_col_min:ker_col_max].reshape(-1)

                            gvars = gvars_array[in_chan_idx, in_row_idx_min:in_row_idx_max + 1,
                                    in_col_idx_min:in_col_idx_max + 1].reshape(-1)
                        ...
                        if need_refine and (relu_idx >= 1) and (out_lb * out_ub < 0) and (
                                time.time() - mip_refine_time_start < mip_refine_timeout):
                            candidates.append(v.VarName)
            ...
            if False and need_refine and relu_idx >= 1 and (time.time() - mip_refine_time_start < mip_refine_timeout):
                ...
                lb_refined_sum, ub_refined_sum, refined_num = 0., 0., 0
                for (vlb, vub, refined), (out_chan_idx, out_row_idx, out_col_idx) in zip(solver_result,
                                                                                         candidate_neuron_ids):
                    if refined:
                        ...
                        for col_idx, pre_var in enumerate(row):
                            ...
                            if pre_lb >= 0:
                                ...
                                a = m.model.addVar(vtype=grb.GRB.BINARY,
                                                   name=f'aReLU{relu_idx}_[{chan_idx},{row_idx},{col_idx}]')
    ...
    if last_relu_layer_refined:
        ...
        lb_refined, ub_refined = m.net.compute_bounds(x=(x,), C=m.c,
                                                      method='CROWN-optimized', reference_bounds=reference_bounds,
                                                      bound_upper=False)
    ...
    lb_refined, ub_refined, pre_relu_indices = m.get_candidate(m.net, lb_refined, lb_refined + float(
        'inf'))  # primals are better upper bounds
