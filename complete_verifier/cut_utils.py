...
def parse_cplex_cuts(cut_bin, var_names, relu_layer_names, pre_relu_layer_names):
    ...
    try:
        ...
        num_rows, num_elements, row_begin_idx_offset, rhs_values_offset, row_indices_offset, row_values_offset = struct.unpack(
            '6i', cut_mbr[4:28])
            ...
            for cut_var_idx in range(row_begin_start, row_begin_end):
                ...
                if "inp_" in var_name:
                    ...
                    if layer_name not in pre_relu_layer_names:
                        ...
                        print(
                            f"Warning: var{var_name} not in pre_relu_layer_names {pre_relu_layer_names}, skip this constraint!")
                        skip = True
                        break
    ...
    for layer, d in enumerate(cut_raw):
        ...
        for con in kact_cons:
            ...
            for i in range(con['cons'].shape[0]):
                ...
                if False and (
                        (con['cons'][i][1:1 + k] != 0).sum() == 1 and (con['cons'][i][1 + k:1 + k * 2] != 0).sum() == 1
                        and np.absolute(con['cons'][i][1:1 + k]).argmax() == np.absolute(
                    con['cons'][i][1 + k:1 + k * 2]).argmax()):
    ...
    if net.cplex_processes is None and net.processes is not None:
        ...
        for pidx in net.processes:
            ...
            if '_logfile' in net.processes[pidx] and net.processes[pidx]['_logfile'] is not None:
                try:
                    os.close(net.processes[pidx]['_logfile'])
                except:
                    pass
...
def cplex_update_general_beta(net, selected_domains):
    for sd in selected_domains:
        if ("general_betas" in sd.split_history and sd.split_history[
            "cut_timestamp"] != net.net.cut_timestamp and net.net.cut_module.general_beta is not None) or \
                ("general_betas" not in sd.split_history and getattr(net.net, "cut_module",
                                                                     None) is not None and net.net.cut_module.general_beta is not None):
            # copy initial general beta from the latest cut module
            # ensure that for each domain the stack batch = 1
            sd.split_history["general_betas"] = net.cutter.beta_init * torch.ones_like(
                net.net.cut_module.general_beta[net.net.final_name][:, :, 0:1, :])
            # need to attach the latest timestamp
            sd.split_history["cut_timestamp"] = net.net.cut_timestamp
