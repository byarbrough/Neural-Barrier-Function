...
from .attack_pgd import attack_with_general_specs
from .branching_domains import ReLUDomain
...
def find_promising_domains(net, adv_pool, dive_domains, candidates, start_iter, max_dive_fix, min_local_free):
    ...
    if find_promising_domains.current_method == 'top-down':
        ...
        if max_dive_fix_ratio > 0 and find_promising_domains.topdown_status == "infeas":
            ...
            print(
                f"### topdown most inf! reduce max_dive_fix_ratio to {new_max_dive_fix_ratio}, now fix: {max_dive_fix}")
    ...
    if find_promising_domains.current_method == 'bottom-up':
        ...
        if min_local_free_ratio > 0. and find_promising_domains.bottomup_status == "timeout":
            ...
            print(
                f"### bottom-up most timeout! decrease min_local_free_ratio to {new_min_local_free_ratio}, fix: {min_local_free}")
    ...
    if find_promising_domains.current_method == 'top-down':
        ...
        for i in range(n_domains - 1):
            ...
            if i < len(adv_pool.adv_pool):
                uncommon_split, _ = adv_pool.get_ranked_activation_pattern(n_activations=min_local_free,
                                                                           find_uncommon=True, random_keep=True)
        ...
        for i in range(n_domains):
            ...
            if i < len(dive_domains):
                ...
                if max_dive_fix > len(s):
                    adv_s, adv_c = adv_pool.get_ranked_activation_pattern(n_activations=max_dive_fix - len(s),
                                                                          blacklist=s, random_keep=True)
...
def beam_alpha_crown(net, splits, coeffs):
    ...
    for m in net.net.relus:
        for spec_name in slope[m.name].keys():
            m.alpha[spec_name] = slope[m.name][spec_name].repeat(1, 1, batch, *(
                        [1] * (slope[m.name][spec_name].ndim - 3))).detach().requires_grad_()
    ...
    net.net.set_bound_opts(
        {'optimize_bound_args': {'iteration': init_iteration, 'enable_beta_crown': False, 'enable_alpha_crown': True,
    ...
    if finalize and beam_mip_attack.started:
        ...
        if solutions is not None and solutions.size(0) != 0:
            ...
            attack_ret, attack_images, attack_margin, _ = attack_with_general_specs(
                ...
                list_target_label_arrays=solutions.size(0) * [((net.c[0].cpu(), torch.tensor([0.])),)],
                # (prop_mat, rhs).
                initialization="none")
            ...
            print(
                f'mip ub: {min(mip_margins.view(-1)).item()} -> mip ub (PGD): {min(attack_margin.view(-1)).item()}, best adv in pool: {adv_pool.adv_pool[0].obj}, worst {adv_pool.adv_pool[-1].obj}')

        if not attack_success:
            # Run new MIPs based on selected domains.
            splits, coeffs, advs, act_patterns = find_promising_domains(net, adv_pool, dive_domains, mip_multi_proc,
                                                                        submip_start_iteration, max_dive_fix,
                                                                        min_local_free)
            if len(splits) > 0:
                ...
                if arguments.Config["bab"]["attack"]["refined_mip_attacker"]:
                    ...
                    if refined_batch_size is None or batch <= refined_batch_size:
                        ...
                        while start_batch < batch:
                            rlbs, rubs = beam_alpha_crown(net, splits[start_batch: end_batch],
                                                          coeffs[start_batch: end_batch])
                        ...
                        for relu_idx in range(len(refined_lower_bounds)):
                            ...
                            assert refined_lower_bounds[relu_idx].size(
                                0) == batch, f"refined_batch_size process wrong, {relu_idx}, {refined_lower_bounds[relu_idx].size(0)} != {batch}!"

                net.update_mip_model_fix_relu(splits, coeffs, target=None,
                                              async_mip=True, best_adv=advs, adv_activation_pattern=act_patterns,
                                              refined_lower_bounds=refined_lower_bounds,
                                              refined_upper_bounds=refined_upper_bounds)
...
def bab_attack(dive_domains, net, batch, pre_relu_indices, growth_rate, fix_intermediate_layer_bounds=True,
               adv_pool=None, max_dive_fix=float("inf"), min_local_free=0):
    ...
    mask, lAs, orig_lbs, orig_ubs, slopes, betas, _, selected_domains, cs, rhs = domains_params
    ...
    if len(merged_decisions[0]) == 0:
        print(
            "No beam search domains left, attack failed, please increase search candidates by increasing beam_candidates.")
    ...
    domains_params = add_dive_domain_from_dive_decisions(selected_domains, merged_decisions, mask=mask,
                                                         device=net.x.device)
    ...
    dom_ub, dom_lb, lAs, dom_lb_all, dom_ub_all, slopes, split_history, betas, cs = [], [], [], [], [], [], [], [], []
    primals = None
    ...
    for i in range(len(batch_boundaries) - 1):
        ...
        ret = net.get_lower_bound(batch_orig_lbs, batch_orig_ubs, batch_split, slopes=batch_slopes,
                                  ...
                                  fix_intermediate_layer_bounds=fix_intermediate_layer_bounds, single_node_split=True,
                                  intermediate_betas=None)
        batch_dom_ub, batch_dom_lb, _, batch_lAs, batch_dom_lb_all, batch_dom_ub_all, batch_slopes, batch_split_history, batch_betas, _, batch_primals, batch_cs = ret
        ...
        # Slopes must be handled specially as a nested dictionary.
        single_item_slopes = [defaultdict(dict) for i in range(len(batch_history))]
        for m_name in list(orig_slopes.keys()):
            for spec_name in orig_slopes[m_name]:
                # Split across batch dimension.
                all_slopes = batch_slopes[m_name][spec_name].split(1, dim=2)
                for i, s in enumerate(all_slopes):
                    single_item_slopes[i][m_name][spec_name] = all_slopes[i]
        slopes.extend(single_item_slopes)
    ...
    if adv_pool is not None:
        ...
        adv_pool.print_pool_status()
        print(
            f'ub: {min(dom_ub)} -> ub (PGD): {min(attack_margin)}, best adv in pool: {adv_pool.adv_pool[0].obj}, worst {adv_pool.adv_pool[-1].obj}')
    ...
    print(
        f'Total time: {total_time:.4f}\t pickout: {pickout_time:.4f}\t decision: {decision_time:.4f}\t dive: {dive_time:.4f}\t get_bound: {solve_time:.4f}\t add_domain: {add_time:.4f}')

    dive_domains = probabilistic_select_domains(dive_domains, max(candidates_number, arguments.Config["solver"]["mip"][
        "parallel_solvers"]))
    ...
    mip_ub, solutions = beam_mip_attack(net, adv_pool, dive_domains, submip_start_iteration, max_dive_fix,
                                        min_local_free, finalize=len(dive_domains) == 0)
    ...
    print('{} domains visited'.format(diving_Visited))
...
def add_dive_domain_from_dive_decisions(dive_domains, dive_decisions, mask=None, device='cuda'):
    ...
    for di, dive_d in enumerate(dive_domains):
        ...
        expand_lb = [dive_d.lower_all[i].repeat(repeats, *([1] * (dive_d.lower_all[i].ndim - 1))) for i in
                     range(len(dive_d.lower_all))]
        expand_ub = [dive_d.upper_all[i].repeat(repeats, *([1] * (dive_d.upper_all[i].ndim - 1))) for i in
                     range(len(dive_d.upper_all))]
        ...
        for i in range(len(expand_lb)):
            ...
            if len(selected_one_idx) == 0 and len(selected_zero_idx) == 0:
                continue
            expand_lb[i].view(repeats, -1)[
                one_coeffs[:, 0][selected_one_idx.squeeze()], one_idx[selected_one_idx.squeeze()][:, 1]] = 0
            expand_ub[i].view(repeats, -1)[
                zero_coeffs[:, 0][selected_zero_idx.squeeze()], zero_idx[selected_zero_idx.squeeze()][:, 1]] = 0
        ...
        for m_name in list(new_dive_d.slope.keys()):
            for spec_name in new_dive_d.slope[m_name]:
                tmp_slope[m_name] = {spec_name: new_dive_d.slope[m_name][spec_name].repeat(1, 1, repeats, *(
                            [1] * (new_dive_d.slope[m_name][spec_name].ndim - 3)))}
    ...
    for m_name in list(slopes_all[0].keys()):
        for spec_name in slopes_all[0][m_name]:
            ret_s[m_name] = {
                spec_name: torch.cat([slopes_all[i][m_name][spec_name] for i in range(len(slopes_all))], dim=2)}
    ...
    for i in range(batch):
        ...
        if lb[i] < decision_thresh:
            ...
            if not infeasible:
                ...
                dive_d = ReLUDomain(lA[i], lb[i].item(), ub[i].item(), lb_all[i], ub_all[i], slope[i], beta[i],
                                    selected_domains[i].depth + 0, split_history=split_history[i],
                                    # depth has been added during diving.
                                    history=new_history,
...
def pickout_dive_domains(domains, batch, device='cuda', diving=False):
    ...
    for j in range(len(lower_bounds) - 1):  # Exclude the final output layer.
        new_masks.append(
            torch.logical_and(lower_bounds[j] < 0, upper_bounds[j] > 0).view(lower_bounds[0].size(0), -1).float())
    ...
    # slopes_all is a two level dictionary.
    for k, v_map in slopes_all[0].items():
        slopes[k] = {}
        for kk, vv in v_map.items():
            slopes_for_this_layer = []
            for s in slopes_all:
                # Go over the batch dimension and add all slopes for this key pair.
                slopes_for_this_layer.append(s[k][kk])
            slopes[k][kk] = torch.cat(slopes_for_this_layer, dim=2).to(device=device, non_blocking=True)
