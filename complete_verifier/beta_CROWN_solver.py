...
from .lp_mip_solver import reduction_str2func
# from .lp_mip_solver import construct_mip_with_model
...
def gradient_descent_update(loss_fcn, var, lb=None, ub=None, num_iter=200):
...
class LiRPAConvNet:
    ...
    def get_lower_bound(self, pre_lbs, pre_ubs, split, slopes=None, betas=None, history=None,
                        fix_intermediate_layer_bounds=True,
                        split_history=None, single_node_split=True, intermediate_betas=None, cs=None,
                        decision_thresh=None, rhs=0,
    ...
    def transfer_to_cpu(self, net, non_blocking=True, opt_intermediate_beta=False, transfer_items="all"):
        ...
        if transfer_items == "all":
            if self.interm_transfer:
                for cpu_layer, layer in zip(cpu_net.perturbed_optimizable_activations,
                                            net.perturbed_optimizable_activations):
                    # For get_candidate_parallel.
                ...
                if hasattr(layer, 'sparse_beta') and layer.sparse_beta is not None:
                    if arguments.Config["solver"]["beta-crown"]['enable_opt_interm_bounds']:
                        ...
                        for key in layer.sparse_beta.keys():
                            cpu_layer.sparse_beta[key] = layer.sparse_beta[key].to(device='cpu',
            ...
            if opt_intermediate_beta and net.best_intermediate_betas is not None:
                ...
                for split_layer, all_int_betas_this_layer in net.best_intermediate_betas.items():
                    ...
                    for intermediate_layer, this_layer_intermediate_betas in all_int_betas_this_layer['single'].items():
                        cpu_net.best_intermediate_betas[split_layer]['single'][intermediate_layer]['lb'] = \
                        this_layer_intermediate_betas['lb'].to(device='cpu', non_blocking=non_blocking)
                        cpu_net.best_intermediate_betas[split_layer]['single'][intermediate_layer]['ub'] = \
                        this_layer_intermediate_betas['ub'].to(device='cpu', non_blocking=non_blocking)
                        transfer_size['itermediate_beta'] += this_layer_intermediate_betas['lb'].numel() * 2
        print(
            f'Tensors transferred: {" ".join("{}={:.4f}M".format(k, v / 1048576) for (k, v) in transfer_size.items())}')
        return cpu_net
    ...
    def get_primal_upper_bound(self, A):
        ...
        elif input_A_lower.dim() == 3:
            x_lb = x_lb.repeat(batch, 1)
            x_ub = x_ub.repeat(batch, 1)
            input_primal = x_lb.clone().detach()
            index = input_A_lower.squeeze(1) < 0
            input_primal[index] = x_ub[index]

            num_samples = arguments.Config['alg_options']['bab']['get_upper_bound_samples']
            lb, ub = self.x.ptb.x_L, self.x.ptb.x_U
            lb, ub = lb.repeat(num_samples, 1), ub.repeat(num_samples, 1)
            random_samples = (ub-lb)*torch.rand(num_samples, 1).to(self.device)+lb

            aug_samples = torch.cat((input_primal, random_samples), dim=0)
            output = self.model_ori(aug_samples).matmul(self.c[0].transpose(-1, -2))

            ind = torch.argsort(output.view(-1))[:batch]
            input_primal = aug_samples[ind]

            def loss_fcn(x):
                output = self.model_ori(x).matmul(self.c[0].transpose(-1, -2))
                return output.sum()

            loss, updated_input = gradient_descent_update(loss_fcn, input_primal, lb=self.x.ptb.x_L, ub=self.x.ptb.x_U, num_iter=200)

            # original:
            # input_primal[input_A_lower.squeeze(1) < 0] = x_ub[input_A_lower.squeeze(1) < 0]
        return updated_input.detach(), self.model_ori(input_primal).matmul(self.c[0].transpose(-1, -2))
    ...
    def get_lA_parallel(self, model, preserve_mask=None, tot_cells=None, to_cpu=False):
        ...
        if preserve_mask is not None:
            ...
            for this_relu in model.perturbed_optimizable_activations:
                lA.append(this_relu.lA.transpose(0, 1).to(device='cpu',
                                                          non_blocking=False) if to_cpu else this_relu.lA.squeeze(0))
    ...
    def get_slope(self, model, only_final=False):
        ...
        kept_layer_names.extend(
            filter(lambda x: len(x.strip()) > 0, arguments.Config["bab"]["optimized_intermediate_layers"].split(",")))
    ...
    def set_slope(self, model, slope, intermediate_refinement_layers=None, diving_batch=0, set_all=False):
        cleanup_intermediate_slope = isinstance(intermediate_refinement_layers, list) and len(
            intermediate_refinement_layers) == 0
                        ...
                        if intermediate_refinement_layers is not None or spec_name in kept_layer_names or set_all:
                            if len(slope) - diving_batch > 0:
                                ...
                                m.alpha[spec_name] = torch.cat(
                                    [slope[i][m.name][spec_name] for i in range(len(slope) - diving_batch)], dim=2)
                                # Duplicate for the second half of the batch.
                                m.alpha[spec_name] = m.alpha[spec_name].repeat(1, 1, 2, *(
                                            [1] * (m.alpha[spec_name].ndim - 3))).detach().requires_grad_()
                            if diving_batch > 0:
                                # create diving alpha
                                diving_alpha = torch.cat(
                                    [slope[i][m.name][spec_name] for i in range(len(slope) - diving_batch, len(slope))],
                                    dim=2)
                                if diving_batch == len(slope):
                                    m.alpha[spec_name] = diving_alpha.detach().requires_grad_()
                                else:
                                    m.alpha[spec_name] = torch.cat([m.alpha[spec_name], diving_alpha],
                                                                   dim=2).detach().requires_grad_()
                                del diving_alpha
            ...
            if isinstance(m, BoundRelu):
                ...
                if arguments.Config["solver"]["beta-crown"]['enable_opt_interm_bounds']:
                    ...
                    for key in m.alpha.keys():
                        m.sparse_beta[key] = torch.zeros(size=(batch, max_splits_per_layer[mi]),
                                                         dtype=torch.get_default_dtype(), device='cpu',
                                                         requires_grad=False)
                        m.sparse_beta_loc[key] = torch.zeros(size=(batch, max_splits_per_layer[mi]), dtype=torch.int64,
                                                             device='cpu', requires_grad=False)
                        m.sparse_beta_sign[key] = torch.zeros(size=(batch, max_splits_per_layer[mi]),
                                                              dtype=torch.get_default_dtype(), device='cpu',
                                                              requires_grad=False)
                    ...
                    for key in m.sparse_beta.keys():
                        m.sparse_beta[key] = m.sparse_beta[key].repeat(2, 1).detach().to(device=self.net.device,
                                                                                         non_blocking=True).requires_grad_()
                else:
                    m.sparse_beta = torch.zeros(size=(batch, max_splits_per_layer[mi]), dtype=torch.get_default_dtype(),
                                                device='cpu', requires_grad=False)
                    m.sparse_beta_loc = torch.zeros(size=(batch, max_splits_per_layer[mi]), dtype=torch.int64,
                                                    device='cpu', requires_grad=False)
                    m.sparse_beta_sign = torch.zeros(size=(batch, max_splits_per_layer[mi]),
                                                     dtype=torch.get_default_dtype(), device='cpu', requires_grad=False)
                    # Load beta from history.
                    for bi in range(batch):
                        if betas is not None and betas[bi] is not None:
                            # First dimension of betas is batch, second dimension is relu layer, third dimension is saved betas.
                    ...
                    # This is the beta variable to be optimized for this layer.
                    m.sparse_beta = m.sparse_beta.repeat(2, 1).detach().to(device=self.net.device,
                                                                           non_blocking=True).requires_grad_()

                assert betas is None or batch + diving_batch == len(betas)
                if diving_batch != 0:
                    m.diving_sparse_beta = torch.zeros(size=(diving_batch, max_splits_per_layer[mi]),
                                                       dtype=torch.get_default_dtype(), device='cpu',
                                                       requires_grad=False)
                    m.diving_sparse_beta_loc = torch.zeros(size=(diving_batch, max_splits_per_layer[mi]),
                                                           dtype=torch.int64, device='cpu', requires_grad=False)
                    m.diving_sparse_beta_sign = torch.zeros(size=(diving_batch, max_splits_per_layer[mi]),
                                                            dtype=torch.get_default_dtype(), device='cpu',
                                                            requires_grad=False)
    ...
    def update_bounds_parallel(
        ...
        if beta is None:
            beta = arguments.Config["solver"]["beta-crown"][
                "beta"]  # might need to set beta False in FSB node selection
        ...
        best_intermediate_betas = [defaultdict(dict) for _ in range(
            batch * 2 + diving_batch)]  # Each key is corresponding to a pre-relu layer, and each value intermediate beta values for neurons in that layer.
        ...
        if beta:
            ...
            for bi in range(batch):
                ...
                for mi, layer_splits in enumerate(history[bi]):
                    splits_per_example[bi, mi] = len(layer_splits[0]) + int(
                        d == mi)  # First element of layer_splits is a list of split neuron IDs.
            ...
            if diving_batch != 0:
                ...
                for dbi in range(diving_batch):
                    ...
                    for mi, diving_layer_splits in enumerate(history[dbi + batch]):
                        diving_splits_per_example[dbi, mi] = len(
                            diving_layer_splits[0])  # First element of layer_splits is a list of split neuron IDs.
            ...
            self.reset_beta(batch, betas=betas, max_splits_per_layer=max_splits_per_layer,
            ...
            if arguments.Config["solver"]["beta-crown"]['enable_opt_interm_bounds']:
                for bi in range(batch):
                    ...
                    for mi, (split_locs, split_coeffs) in enumerate(history[bi]):
                        ...
                        for key in self.net.relus[mi].sparse_beta.keys():
                            self.net.relus[mi].sparse_beta_sign[key][bi, :split_len] = torch.as_tensor(split_coeffs,
                                                                                                       device='cpu',
                                                                                                       dtype=torch.get_default_dtype())
                            self.net.relus[mi].sparse_beta_loc[key][bi, :split_len] = torch.as_tensor(split_locs,
                                                                                                      device='cpu',
                                                                                                      dtype=torch.int64)
                ...
                for bi in range(batch):
                    ...
                    for mi, (split_locs, split_coeffs) in enumerate(history[bi]):
                        split_len = len(split_locs)
                        self.net.relus[mi].sparse_beta_sign[bi, :split_len] = torch.as_tensor(split_coeffs,
                                                                                              device='cpu',
                                                                                              dtype=torch.get_default_dtype())
                        self.net.relus[mi].sparse_beta_loc[bi, :split_len] = torch.as_tensor(split_locs, device='cpu',
                                                                                             dtype=torch.int64)
                        # Add current decision for positive splits.
                        if mi == d:
            ...
            if diving_batch > 0:
                ...
                for dbi in range(diving_batch):
                    for mi, (split_locs, split_coeffs) in enumerate(history[dbi + batch]):
                        split_len = len(split_locs)
                        self.net.relus[mi].diving_sparse_beta_sign[dbi, :split_len] = torch.as_tensor(split_coeffs,
                                                                                                      device='cpu',
                                                                                                      dtype=torch.get_default_dtype())
                        self.net.relus[mi].diving_sparse_beta_loc[dbi, :split_len] = torch.as_tensor(split_locs,
                                                                                                     device='cpu',
                                                                                                     dtype=torch.int64)
                for m in self.net.relus:
            ...
            if self.net.cut_used and getattr(self.net, "cut_module", None) is not None:
                ...
                general_beta = self.cutter.beta_init * torch.ones((2, 1, batch_size, num_constrs),
                ...
                if split_history is not None:
                    ...
                    for batch_sh, sh in enumerate(split_history):
                        if "general_betas" in sh and sh["cut_timestamp"] == self.net.cut_timestamp:
                            ...
                            general_beta[:, :, batch_sh + batch: batch_sh + batch + 1, :] = sh[
                                "general_betas"].detach().clone()
        ...
        with torch.no_grad():
            ...
            zero_indices_batch = [torch.as_tensor(t).to(device=self.net.device, non_blocking=True) for t in
                                  zero_indices_batch]
            zero_indices_neuron = [torch.as_tensor(t).to(device=self.net.device, non_blocking=True) for t in
                                   zero_indices_neuron]
            ...
            for d in range(len(lower_bounds)):
                ...
                if len(zero_indices_batch[d]):
                    ...
                    upper_bounds[d][:2 * batch].view(2 * batch, -1)[
                        zero_indices_batch[d] + batch, zero_indices_neuron[d]] = 0.0
        ...
        ptb = PerturbationLpNorm(norm=self.x.ptb.norm, eps=self.x.ptb.eps,
                                 x_L=self.x.ptb.x_L[0].expand(batch * 2 + diving_batch,
                                                              *[-1] * (self.x.ptb.x_L.ndim - 1)),
                                 x_U=self.x.ptb.x_U[0].expand(batch * 2 + diving_batch,
                                                              *[-1] * (self.x.ptb.x_L.ndim - 1)))
        ...
        if fix_intermediate_layer_bounds:
            ...
            kept_layer_names = list(filter(lambda x: len(x.strip()) > 0,
                                           arguments.Config["bab"]["optimized_intermediate_layers"].split(",")))
        ...
        print(
            f'This batch time : update_bounds func: {func_time:.4f}\t prepare: {prepare_time:.4f}\t bound: {bound_time:.4f}\t transfer: {transfer_time:.4f}\t finalize: {finalize_time:.4f}')
        print(
            f'Accumulated time: update_bounds func: {total_func_time:.4f}\t prepare: {total_prepare_time:.4f}\t bound: {total_bound_time:.4f}\t transfer: {total_transfer_time:.4f}\t finalize: {total_finalize_time:.4f}')
    ...
    def update_bounds_cut_naive(
        ...
        with torch.no_grad():
            if not batchwise_out:
                ...
                return ub[-1], lb[-1], None, None, None, mask[0], lA[
                    0], lb, ub, None, slope_opt, history, new_split_history
            else:
                # Move tensors to CPU for all elements in this batch.
                start_transfer_time = time.time()
                lb = lb.to(device='cpu')
    ...
    def set_cuts(self, A, x, lower_bounds, upper_bounds, use_float64_in_last_iteration=False):
        ...
        if cuts is None and not arguments.Config["bab"]["cut"]["cplex_cuts"]:
            print(
                "Warning: Cuts should either be automatically generated by enabling specifying --cut_method or manually given by --tmp_cuts")
            exit()

    def build_the_model(self, input_domain, x, data_lb=None, data_ub=None, vnnlib=None,
                        stop_criterion_func=stop_criterion_sum(0), bounding_method=None):
        """
        ...
        if bounding_method == "alpha-crown":
            ...
            if arguments.Config["attack"]["pgd_order"] == "middle" and vnnlib is not None:
                ...
                verified_status, verified_success, attack_images, _, _ = attack(
                    self.model_ori, x, data_lb, data_ub, [vnnlib], verified_status, verified_success,
                    crown_filtered_constraints)
            ...
            if arguments.Config["solver"]["prune_after_crown"]:
                ...
                for relu in self.net.relus:
                    if relu.alpha is not None and self.net.final_name in relu.alpha:
                        relu.alpha[self.net.final_name] = relu.alpha[self.net.final_name][:,
                                                          unverified_label_mask].detach()
                prune_after_crown_used = True
                prune_after_crown_overhead += time.time() - stime
                print('prune_after_crown optimization in use: original label size =', final_layer_lb.shape[0],
                      'pruned label size =', len(unverified_label_mask))
        ...
        if arguments.Config["bab"]["cut"]["enabled"] and arguments.Config["bab"]["cut"][
            "cplex_cuts"] and self.mip_building_proc is None:
            ...
            self.cutter = Cutter(self, A, x, number_cuts=arguments.Config["bab"]["cut"]["number_cuts"],
                                 device=self.net.device)
        ...
        for m in self.net.relus:
            for spec_name, alpha in m.alpha.items():
                def not_setting_alpha():
                    print(
                        f"not setting layer {m.name} start_node {spec_name} because shape mismatch ({alpha.size()} != {reference_alpha.size()})")
                ...
                elif all([si == sj or ((d == 2) and sj == 1) for d, (si, sj) in
                          enumerate(zip(alpha.size(), reference_alpha.size()))]):
        ...
        for now_batch in range(tot_batches):
            ...
            if refined_lower_bounds is not None and refined_upper_bounds is not None:
                ...
                for i, layer in enumerate(self.net.relus):
                    ...
                    intermediate_layer_bounds[nd] = [
                        refined_lower_bounds[i][now_batch * target_batch_size: (now_batch + 1) * target_batch_size],
            ...
            self.net.init_slope((batch_expand,), share_slopes=share_slopes,
                                ...
                                intermediate_layer_bounds=intermediate_layer_bounds,
                                activation_opt_params=activation_opt_params, skip_bound_compute=True)

            all_slope_initialized = self.copy_alpha(
                reference_slopes,
                num_targets=min((now_batch + 1) * target_batch_size, self.c.shape[0]) - now_batch * target_batch_size,
            ...
            if all_slope_initialized and arguments.Config["solver"]["multi_class"]["skip_with_refined_bound"] is True:
                ...
                print(
                    'restore to original setting since some slopes are not initialized yet or being asked not to skip')
            ...
            batch_lb, batch_ub, pre_relu_indices = self.get_candidate(self.net, lb,
                                                                      lb + np.inf)  # primals are better upper bounds
            ...
            if not opt_intermediate_beta:
                ...
                new_slope_opt = {}
                kept_layer_names = [self.net.final_name]
                kept_layer_names.extend(filter(lambda x: len(x.strip()) > 0,
                                               arguments.Config["bab"]["optimized_intermediate_layers"].split(",")))
    ...
    def get_neuron_primal(self, input_primal, lb, ub, slope_opt=None):
        ...
        for layer in self.layers:
            if not isinstance(layer, nn.ReLU):
                ...
                if self.net.relus[relu_idx].sparse_beta is not None and self.net.relus[
                    relu_idx].sparse_beta.nelement() != 0:
                    ...
                    primal.view(batch_size, -1)[beta_loc_tuple] = primal_values[-1].view(batch_size, -1)[
                                                                      beta_loc_tuple] * beta_c
    ...
    def create_mip_building_proc(self, x):
        ...
        for name, layer in self.net._modules.items():
            layer_lower = layer.lower.clone().cpu() if hasattr(layer, 'lower') and isinstance(layer.lower,
                                                                                              torch.Tensor) else None
            layer_upper = layer.upper.clone().cpu() if hasattr(layer, 'upper') and isinstance(layer.upper,
                                                                                              torch.Tensor) else None
            if layer_lower is not None or layer_upper is not None:
                intermediate_bounds[name] = [layer_lower,
                                             layer_upper]  # Save its intermediate layer bounds in a dictionary.
