...
def reshape_bounds(lower_bounds, upper_bounds, y, global_lb=None):
    with torch.no_grad():
        last_lower_bounds = torch.zeros(size=(1, lower_bounds[-1].size(1) + 1), dtype=lower_bounds[-1].dtype,
                                        device=lower_bounds[-1].device)
        last_upper_bounds = torch.zeros(size=(1, upper_bounds[-1].size(1) + 1), dtype=upper_bounds[-1].dtype,
                                        device=upper_bounds[-1].device)
        ...
        if global_lb is not None:
            last_global_lb = torch.zeros(size=(1, global_lb.size(1) + 1), dtype=global_lb.dtype,
                                         device=global_lb.device)
...
def get_pgd_acc(model, X, labels, eps, data_min, data_max, batch_size):
    ...
    while start < total:
        ...
        best_deltas, last_deltas = attack_pgd(model, X=batch_X, y=batch_labels, epsilon=float("inf"), alpha=alpha,
                                              ...
                                              upper_limit=data_ub, lower_limit=data_lb, multi_targeted=True,
        ...
        print(
            f'batch start {start}, batch size {end - start}, clean correct {batch_clean_correct}, robust correct {batch_robust_correct}')
    ...
    print(
        f'data start {arguments.Config["data"]["start"]} end {total}, clean correct {clean_correct}, robust correct {robust_correct}')
...
def get_test_acc(model, input_shape=None, X=None, labels=None, batch_size=256):
    ...
    if X is None and labels is None:
        ...
        if input_shape == (3, 32, 32):
            testset = torchvision.datasets.CIFAR10(root=database_path, train=False, download=True,
                                                   transform=transforms.Compose([transforms.ToTensor(), normalize]))
        elif input_shape == (1, 28, 28):
            testset = torchvision.datasets.MNIST(root=database_path, train=False, download=True,
                                                 transform=transforms.Compose([transforms.ToTensor(), normalize]))
        else:
        ...
        for data in testloader:
            ...
            if print_first_batch:
                ...
                for i in range(min(outputs.size(0), 10)):
                    print(
                        f"Image {i} norm {images[i].abs().sum().item()} label {labels[i].item()} correct {labels[i].item() == outputs[i].argmax().item()}\nprediction {outputs[i].cpu().numpy()}")
...
def load_generic_dataset(eps_temp=None):
    ...
    runnerup = None
    # Rescale epsilon.
    eps_temp = torch.reshape(eps_temp / torch.tensor(arguments.Config["data"]["std"], dtype=torch.get_default_dtype()),
                             (1, -1, 1, 1))
...
def parse_run_mode():
    ...
    if arguments.Config["general"]["csv_name"] is not None and arguments.Config["specification"]["vnnlib_path"] is None:
        ...
        if arguments.Config["data"]["start"] != 0 or arguments.Config["data"]["end"] != reader.line_num:
            assert 0 <= arguments.Config["data"]["start"] <= reader.line_num and arguments.Config["data"]["end"] > \
                   arguments.Config["data"]["start"], \
                "specified --start or --end out of range: start={}, end={}, total_in_csv={}".format(
                    arguments.Config["data"]["end"], arguments.Config["data"]["start"], reader.line_num)
            print("customized start/end sample from instance {} to {} in {}".format(arguments.Config["data"]["start"],
    ...
    elif arguments.Config["model"]["onnx_path"] is not None and arguments.Config["specification"][
        "vnnlib_path"] is not None:
        ...
        if arguments.Config["specification"]["vnnlib_path"] is None:
            ...
            if arguments.Config["specification"]["epsilon"] is not None:
                perturb_epsilon = torch.tensor(arguments.Config["specification"]["epsilon"],
                                               dtype=torch.get_default_dtype())
            else:
                print('No epsilon defined!')
                perturb_epsilon = None
            X, labels, runnerup, data_max, data_min, perturb_epsilon, target_label = load_verification_dataset(
                perturb_epsilon)
            ...
            vnnlib_all = construct_vnnlib(X, labels, runnerup, data_max, data_min, perturb_epsilon, target_label,
                                          example_idx_list)
            ...
            assert arguments.Config["model"][
                       "input_shape"] is not None, 'vnnlib does not have shape information, please specify by --input_shape'
