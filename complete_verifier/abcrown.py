...
def main():
    ...
    for new_idx, csv_item in enumerate(example_idx_list):
        ...
        if run_mode != 'customized_data':
            if len(csv_item) == 3:
                ...
                model_ori, shape, vnnlib = eval(arguments.Config["model"]["onnx_loader"])(file_root, onnx_path,
                                                                                          vnnlib_path)
            else:
        ...
        if arguments.Config["bab"]["override_timeout"] is not None:
            ...
            arguments.Config["bab"]["timeout"] = new_timeout
        timeout_threshold = arguments.Config["bab"][
            "timeout"]  # In case arguments.Config["bab"]["timeout"] is changed later.
        ...
        if arguments.Config["data"]["dataset"] == 'NN4SYS':
            ...
            if not verified_success and (
                ...
                verified_status, init_global_lb, lower_bounds, upper_bounds, refined_betas = mip(
            ...
            if not verified_success and arguments.Config["general"][
                "complete_verifier"] != "skip" and verified_status != "unknown-mip":
            ...
            if arguments.Config["bab"]["cut"]["enabled"] and arguments.Config["bab"]["cut"][
                "cplex_cuts"] and saved_bounds is not None:
    ...
    if run_mode != 'single_vnnlib':
        ...
        print("############# Summary #############")
        print("Final verified acc: {}% (total {} examples)".format(len(time_verified) / len(example_idx_list) * 100.,
                                                                   len(example_idx_list)))
        print("Problem instances count:", len(time_verified) + len(time_unsafe) + len(time_timeout),
              ", total verified (safe/unsat):", len(time_verified),
              ", total falsified (unsafe/sat):", len(time_unsafe), ", timeout:", len(time_timeout))
        print(
            f"mean time for ALL instances (total {len(time_all_instances)}): {sum(time_all_instances) / (len(time_all_instances) + 1e-5)}, max time: {max(time_all_instances)}")
