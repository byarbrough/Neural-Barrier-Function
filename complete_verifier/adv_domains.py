...
class AdvExamplePool:
    ...
    def add_adv_images(self, adv_images, max_to_add=float("inf")):
        ...
        for adv_i in range(adv_images.size(0)):
            ...
            c_replaced, c_added, c_rejected = self.replace_adv_example(
                AdvExample(adv_images[adv_i], pred[adv_i].item(), example_activations), c_replaced, c_added, c_rejected)
        print('Number of adv examples replaced/added/rejected: {}/{}/{}'.format(c_replaced, c_added, c_rejected))

    def print_pool_status(self):
        print(
            'Current adv_pool statistic: length {}, variance {}, Manhattan distance mean {}, min {}, max {}, std {:.4f}'.format(
                len(self.adv_pool), *self.get_var()))
    ...
    def replace_adv_example(self, adv_example, c_replaced, c_added, c_rejected):
        if len(self.adv_pool) > arguments.Config["attack"]["pgd_restarts"]:
            current_adv_patterns = torch.stack(
                [torch.cat([ii.flatten() for ii in i.activation_pattern]) for i in self.adv_pool])
    ...
    def get_var(self):
        if len(self.adv_pool) > 0:
            current_adv_patterns = torch.stack(
                [torch.cat([ii.flatten() for ii in i.activation_pattern]) for i in self.adv_pool[1:]])
    ...
    def get_activation_pattern_from_pool(self, prob_threshold=1.0, find_uncommon=False, n_advs=-1):
        ...
        for layer_i in range(self.nlayers):
            ...
            remaining_acc_pattern = ((acc_pattern >= pos_threshold).int() - (acc_pattern <= neg_threshold).int()) * \
                                    self.unstable_mask[layer_i]
            if find_uncommon:
    ...
    def get_ranked_activation_pattern(self, n_activations=-1, blacklist=None, find_uncommon=False, n_advs=-1,
                                      random_keep=False, softmax_temperature=10.0):
        if n_advs == -1:
            # Use entire pool.
            selected_advs = self.adv_pool
        else:
