    ...
    if (os.path.exists(compiled_vnnlib_filename)):
        ...
        if (read_error == False):
            if (curfile_md5 == old_file_md5):
                ...
                print(
                    f"{compiled_vnnlib_suffix} file md5: {old_file_md5} does not match the current vnnlib md5: {curfile_md5}. Regenerating...")
...
def batch_vnnlib(vnnlib):
    ...
    try:
        ...
        print(
            f"Total VNNLIB file length: {len(init_d['x'])}, max property batch size: {batch_size}, total number of batches: {total_batch}")
        ...
        for i in range(total_batch):
            ...
            final_merged_rv.append([init_d['x'][i * batch_size: (i + 1) * batch_size], [
                (init_d['c'][i * batch_size: (i + 1) * batch_size], init_d['rhs'][i * batch_size: (i + 1) * batch_size],
                 true_labels[i * batch_size: (i + 1) * batch_size],
