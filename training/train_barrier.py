...
class Training_Options:
    def __init__(
        self, l1_lambda=None, num_epochs=50, early_stop_tol=1e-10, update_A_freq=1
    ):
...
class CE_Sampling_Options:
    def __init__(
        self, num_ce_samples=20, num_ce_samples_accpm=5, radius=0.1, opt_iter=100
    ):
...
class Trainer:
    ...
    def generate_training_samples(self, save_data_path, sampling_options=None):
        ...
        x_samples, xn_samples = generate_training_data(
            self.system, self.X.set, num_samples_x
        )
    ...
    def train_barrier_fcn_from_samples(
        self,
        save_model_path,
        l1_lambda=None,
        num_epochs=100,
        ...
        num_batches = int(
            np.ceil(max(len(x0_dataset), len(xu_dataset), len(x_dataset)) / batch_size)
        )

        x0_dataloader = torch.utils.data.DataLoader(
            x0_dataset,
            batch_size=int(np.ceil(len(x0_dataset) / num_batches)),
            shuffle=True,
        )
        xu_dataloader = torch.utils.data.DataLoader(
            xu_dataset,
            batch_size=int(np.ceil(len(xu_dataset) / num_batches)),
            shuffle=True,
        )
        x_dataloader = torch.utils.data.DataLoader(
            x_dataset,
            batch_size=int(np.ceil(len(x_dataset) / num_batches)),
            shuffle=True,
        )
    ...
    def sample_counterexamples(self, ce, num_samples, radius=0.1, opt_iter=None):
        ...
        if new_ce_x is None:
            ...
            new_ce_xn = (
                self.problem.system(torch.from_numpy(new_ce_x).to(self.device))
                .detach()
                .cpu()
                .numpy()
            )
        ...
        for i in range(num_ce):
            ...
            if opt_iter is not None:
                ...
                _, new_ce, _ = search_counterexamples(
                    self.problem, type, samples=new_ce, num_iter=opt_iter
                )
    ...
    def train_and_verify(
        ...
        for i in tqdm(range(num_iter), desc='training_iter'):
            ...
            self.train_barrier_fcn_from_samples(
                save_model_path,
                l1_lambda=l1_lambda,
            ...
            if method == 'verification-only':
                ...
                start_time = time.time()
                status, ce, num_queries, solver_time, accpm_results = (
                    self.update_barrier_fcn_by_ACCPM(accpm_opt, timeout=runtime_budget)
                )
                verifier_time = time.time() - start_time
            ...
            new_ce = self.sample_counterexamples(
                ce, num_ce_samples, radius=radius, opt_iter=opt_iter
            )
...
def shrink_and_perturb(net, scaling_factor=0.5, noise_weight=1.0):
    ...
    for i in range(len(params_net)):
        params_net[i].data = (
            scaling_factor * params_net[i].data + noise_weight * params_new_net[i].data
        )
