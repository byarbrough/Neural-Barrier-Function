...
from complete_verifier.complete_verification import (
    incomplete_verifier,
    complete_verifier,
)
...
def bab_verification(nn_model, data_lb, data_ub, C, rhs, yaml_file_path):
    ...
    (
        ...
        nn_model, data, data_lb, data_ub, vnnlib, verified_status, verified_success
    )

    # incompete verifier
    verified_status, global_lb, saved_bounds, saved_slopes, activation_opt_params = (
        incomplete_verifier(
            nn_model, data, data_ub=data_ub, data_lb=data_lb, vnnlib=vnnlib
        )
    )
    ...
    if not verified_success:
        ...
        bab_result = {
            'incomplete_lb': global_lb,
            'bab_lb': bab_lb,
            'bab_ub': bab_ub,
            'status': verified_status,
            'status_lst': status_lst,
            'solver_time': bab_runtime,
            'adv_samples': adv_samples,
            'bab_sol': bab_sol_lst,
            'bab_time_out': is_time_out,
            'data_lb': data_lb,
            'data_ub': data_ub,
        }
        ...
        bab_result = {
            ...
            'status': verified_status,
            'status_lst': None,
            'solver_time': incomplete_verifier_solver_time,
            'adv_samples': None,
            'bab_sol': None,
            'bab_time_out': None,
            'data_lb': data_lb,
