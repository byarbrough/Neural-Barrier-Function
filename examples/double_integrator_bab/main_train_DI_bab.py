...
if __name__ == '__main__':
    ...
    x_lb = np.array([-0.5, -1.0]).astype('float32')
    x_ub = np.array([3.0, 0.5]).astype('float32')
    ...
    xu_min = np.array([1.5, -0.2]).astype('float32')
    ...
    # load dynamics model
    model_config_path = os.path.join(script_directory, 'model_data/doubleIntegrator.json')
    ...
    nn_controller_path = os.path.join(script_directory, 'model_data/doubleIntegratorCorrect.pth')
