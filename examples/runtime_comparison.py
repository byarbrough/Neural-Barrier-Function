...
if __name__ == '__main__':
    ...
    for i in range(8):
        accpm_result_path = os.path.join(DI_result_dir, 'train_iter_accpm_' + plot_opt + '_'+str(i)+'.p')
    ...
    for i in range(8):
        verify_result_path = os.path.join(DI_result_dir, 'train_iter_verify_' + plot_opt + '_' + str(i) + '.p')
    ...
    accpm_avg_runtime = [accpm_result['runtime'][i] for i in range(len(accpm_result['runtime'])) if accpm_result['status'][i] == 'feasible']
    if len(accpm_avg_runtime) > 0:
        accpm_mip_avg_runtime = sum(accpm_avg_runtime)/len(accpm_avg_runtime)
    else:
        accpm_mip_avg_runtime = None

    verify_avg_runtime = [verify_result['runtime'][i] for i in range(len(verify_result['runtime'])) if verify_result['status'][i] == 'feasible']
    ...
    accpm_DI_avg_runtime, verify_DI_avg_runtime = accpm_mip_avg_runtime, verify_mip_avg_runtime
    print('Double integrator average runtime: accpm: ', accpm_DI_avg_runtime, ' verify: ', verify_DI_avg_runtime)
    ...
    plt.plot(range(num_exp), [7200]*num_exp, 'k--', label='Time out threshold', linewidth=3)
    ...
    accpm_result = {'status': [], 'timeout_status': [], 'train_time': [], 'verification_time': [], 'runtime': []}
    for i in range(8):
        accpm_result_path = os.path.join(quad_result_dir, 'train_iter_accpm_' + plot_opt + '_'+str(i)+'.p')
        result = torch.load(accpm_result_path, map_location=torch.device('cpu'))
        accpm_result['status'].append(result['training_results']['status'])
        accpm_result['timeout_status'].append(result['training_results']['timeout_status'])
        accpm_result['train_time'].append(sum(result['training_results']['train_time']))
    ...
    verify_result = {'status': [], 'timeout_status': [], 'train_time': [], 'verification_time': [], 'runtime': []}
    for i in range(8):
        verify_result_path = os.path.join(quad_result_dir, 'train_iter_verify_' + plot_opt + '_' + str(i) + '.p')
        result = torch.load(verify_result_path, map_location=torch.device('cpu'))
        verify_result['status'].append(result['training_results']['status'])
        verify_result['timeout_status'].append(result['training_results']['timeout_status'])
        verify_result['train_time'].append(sum(result['training_results']['train_time']))
    ...
    accpm_quad_avg_runtime, verify_quad_avg_runtime = accpm_mip_avg_runtime, verify_mip_avg_runtime
    print('6D quadrotor average runtime: accpm: ', accpm_quad_avg_runtime, ' verify: ', verify_quad_avg_runtime)
    ...
    circle = plt.Line2D([0], [0], marker='o', color='black', markerfacecolor='none', markersize=20, linestyle='None')
    triangle = plt.Line2D([0], [0], marker='^', color='black', markerfacecolor='none', markersize=20, linestyle='None')
