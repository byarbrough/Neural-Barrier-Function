...
if __name__ == '__main__':
    ...
    barrier_dim = 6
    barrier_net = nn.Sequential(nn.Linear(x_dim, 50),
                                nn.ReLU(),
                                nn.Linear(50, 30),
                                nn.ReLU(),
                                nn.Linear(30, 10),
    ...
    for i in range(xx.shape[0]):
        for j in range(xx.shape[1]):
            ...
            U[i,j] = dynamics(input).detach().cpu().numpy()[0,0].astype('float32') - xx[i, j]
            V[i,j] = dynamics(input).detach().cpu().numpy()[0,1].astype('float32') - yy[i, j]
    ...
    for i in range(xx.shape[0]):
        for j in range(xx.shape[1]):
            input = torch.tensor([[xx[i, j], yy[i, j]]])
            zz[i,j] = barrier_fcn_ind_output(input).detach().cpu().numpy()[0].astype('float32')

    level_value = 0.0
    h = plt.contour(xx, yy, zz, [level_value], linewidths = 3.0, colors='black', linestyles='dashed')
    ...
    proxy = [Patch(fill=False, linewidth=3.0, edgecolor='k'), Patch(fill=False, linewidth=3.0, edgecolor='r'), Patch(fill=False, linewidth=3.0, edgecolor='b'),
