...
def agent_policy_to_nn(agent):
    ...
    nn_policy[0].weight.data, nn_policy[0].bias.data = (
        policy.linear1.weight.data,
        policy.linear1.bias.data,
    )
    nn_policy[2].weight.data, nn_policy[2].bias.data = (
        policy.linear2.weight.data,
        policy.linear2.bias.data,
    )
    nn_policy[-1].weight.data, nn_policy[-1].bias.data = (
        policy.mean.weight.data,
        policy.mean.bias.data,
    )
